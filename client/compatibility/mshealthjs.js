...
// In this Javascript file, it contains all the functions that is used to connect to the javascript API and then the HTML
// file is used to call the functions in this javascript API to get the data.

// the below is to make the API call
// authHeader = "Bearer {access_token}" is to get the access token once the user has clicked yes at the login permissions page
(function (global) {
    "use strict";

    var loginUrl, clientId, scope, redirectUri, accessToken, authHeader, apiTemplateUrl, Promise, MicrosoftHealth, logoutUrl;
    ...
    logoutUrl = "https://login.live.com/oauth20_logout.srf?client_id={client_id}&redirect_uri={redirect_uri}";
    // the api path to the ms health
    ...
    MicrosoftHealth.prototype.logout = function () {
      ...
      if (hash) {
          ...
          url = logoutUrl.replace("{client_id}", clientId).replace("{redirect_uri}", encodeURIComponent(redirectUri));
          global.location = url;
      }
    };
